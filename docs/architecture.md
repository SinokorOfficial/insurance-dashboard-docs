...
```text
  ┌─────────────────────── 사내 파일서버 (SMB) ───────────────────────┐
  │  연간보험료 엑셀 (24~26PY)                                        │
  │  보험현황 엑셀 (2016 · 2018~2026 시트)                            │
  │  증권 PDF 약 13,900개                                             │
  │     선체·전쟁 / P&I·FDD / MII·MAP / 컨테이너                      │
  └────────────────────────────┬──────────────────────────────────────┘
                               │
                               ▼
  ┌────────────── 재생성 파이프라인 (Python · 12단계) ────────────────┐
  │  엑셀 → 레코드 → 과거연도 보완 → 증권 PDF 첨부                    │
  │            → 증권 판독(증권 우선) → 공제액 · 컨테이너             │
  └────────────────────────────┬──────────────────────────────────────┘
                               │
                               ▼
                   ┌───────────────────────┐
            ...
            ┌──────────────────┼──────────────────┐
            ▼                  ▼                  ▼
      로컬 대시보드      웹 대시보드        공유용 HTML /
      담당자·수정가능    보기전용·비밀번호   공유 패키지
                               │              오프라인 단일 파일
                               │
        증권 열기 ─────────────┘
        (요청 때마다 사내 파일서버에서 직접 읽음 — 서버에 복사하지 않는다)
```
